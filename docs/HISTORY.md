# Build history (frozen chronological log)

This is the **verbatim** content of `CLAUDE.md` as it stood on 2026-08-24, before it
was reorganized into a topical reference. It is a historical record, kept so no
detail from the build is lost: the founding brief ("PROMPT 0") followed by every
round of build notes, oldest first, exactly as they were written at the time.

Read it accordingly:

- **Later entries supersede earlier ones.** The brief's clothing exclusion, the
  dept_liaison/dept_orderer roles, OTP-on-every-login, the on-campus hosting plan,
  the availability digest, the time-machine page and the warp shockwave were all
  changed or removed by later entries in this same file.
- **The current truth lives in `CLAUDE.md` (topical) and the code.** Where this
  file and `CLAUDE.md` disagree, `CLAUDE.md` wins. One known error is preserved
  here as-written: the final 2026-08-24 entry says a sale between count and review
  blocks approval — `counting/ledger.py` (same commit) exists precisely so it
  does not.
- A 27-line block ("OOS lists hide never-stocked" + "Themes") appears twice in the
  2026-07-27 pre-deploy entry — a paste error, preserved verbatim.
- New round-by-round narrative may be **appended** here; durable rules go in
  `CLAUDE.md`, rationale in `DECISIONS.md`.
